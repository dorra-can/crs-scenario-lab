...
function renderForm(){
  ...
  let curGroup=null;
  FACTORS.forEach(f=>{
    if(f.spouseOnly && !state.spouse) return;
    if(f.group!==curGroup){ curGroup=f.group;
      const gl=document.createElement('div'); gl.className='glabel'; gl.textContent=f.group; baseForm.appendChild(gl); }
...
function renderVars(){
  ...
  let curGroup=null;
  FACTORS.forEach(f=>{
    if(f.spouseOnly && !state.spouse){ delete vary[f.key]; return; }
    if(f.group!==curGroup){ curGroup=f.group;
      const gl=document.createElement('div'); gl.className='glabel'; gl.textContent=f.group; varList.appendChild(gl); }
    const on=!!vary[f.key];
    ...
    if(on){
      ...
      all.onclick=()=>{ vary[f.key]=new Set(f.opts.map(o=>o[0])); renderVars(); recompute(); };
      const none=document.createElement('button'); none.textContent='Clear';
      none.onclick=()=>{ vary[f.key]=new Set(); renderVars(); recompute(); };
      ...
      f.opts.forEach(o=>{
        ...
        chip.onclick=()=>{ const set=vary[f.key]; if(set.has(o[0]))set.delete(o[0]); else set.add(o[0]); renderVars(); recompute(); };
